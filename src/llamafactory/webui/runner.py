...
from .utils import abort_process, gen_cmd, get_eval_results, get_trainer_info, load_args, save_args, save_cmd, \
    get_cur_datetime, convert_dict_to_env, convert_dict_to_args
...
class Runner:
    ...
    def start_train(self, env: _Environ[str], pre_args: Dict, args: Dict):
        python_path = "cd ~/projects/llama_factory && source ~/pyenvs/llm-env/bin/activate && "
        suf_args = convert_dict_to_args(args)

        nodes_info = pre_args.pop("nodes_info", None)

        master_node_info = nodes_info[0]
        master_node_info['NODE_RANK'] = 0
        pre_env = convert_dict_to_env(master_node_info)
        ...
        for node_rank, node_info in enumerate(nodes_info[1:]):
            node_info['NODE_RANK'] = node_rank
            pre_env = convert_dict_to_env(node_info)
